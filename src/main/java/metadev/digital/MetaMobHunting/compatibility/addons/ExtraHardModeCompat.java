...
public class ExtraHardModeCompat implements IMobHuntCompat, IFeatureHolder {
	...
	public ExtraHardModeCompat() {
        compatPlugin = Bukkit.getPluginManager().getPlugin(SupportedPluginEntities.ExtraHardMode.getName());
    ...
    public void registerFeatures() {
        ...
        // Base plugin
        enabled = MobHunting.getInstance().getConfigManager().enableIntegrationExtraHardMode;
    ...
    public static boolean isEnabledForWorld(World world) {
        if (MobHunting.getInstance().getCompatibilityManager().isCompatibilityLoaded(Bukkit.getPluginManager().getPlugin(SupportedPluginEntities.ExtraHardMode.getName()))) {
            for (String worldName : ((ExtraHardMode) compatPlugin).getModuleForClass(RootConfig.class).getEnabledWorlds())
