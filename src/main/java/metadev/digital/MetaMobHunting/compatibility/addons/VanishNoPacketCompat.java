...
public class VanishNoPacketCompat implements IMobHuntCompat, IFeatureHolder {
    ...
    private static String sMin, sMax, pMin = "3.20.2", pMax;
    private static FeatureList features;

    // ****** Plugin Specific ******
    public VanishNoPacketCompat() {
        compatPlugin = Bukkit.getPluginManager().getPlugin(SupportedPluginEntities.VanishNoPacket.getName());
    ...
    public void registerFeatures() {
        ...
        // Base plugin
        enabled = MobHunting.getInstance().getConfigManager().enableIntegrationVanishNoPacket;
    ...
    public static VanishPlugin getVanishNoPacket() {
        return (VanishPlugin) compatPlugin;
    }

    public static boolean isVanishedModeEnabled(Player player) {
        if (MobHunting.getInstance().getCompatibilityManager().isCompatibilityLoaded(Bukkit.getPluginManager().getPlugin(SupportedPluginEntities.VanishNoPacket.getName())))
