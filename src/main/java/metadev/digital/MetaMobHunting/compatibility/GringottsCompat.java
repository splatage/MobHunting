...
public class GringottsCompat {
	...
	public GringottsCompat() {
		if (!isEnabledInConfig()) {
			MessageHelper.warning("Compatibility with Gringotts is disabled in config.yml");
		} else {
			...
			if (mPlugin.getDescription().getVersion().compareTo(latestSupported) >= 0) {
				...
				MessageHelper.notice("Enabling Compatibility with Gringotts ("
						+ getGringotts().getDescription().getVersion() + ")");
				supported = true;

			} else {
				MessageHelper.warning("Your current version of Gringotts ("
